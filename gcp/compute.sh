...
while IFS= read -r PROJECT_ID; do
    if [[ -n "$PROJECT_ID" ]]; then
        ...
        gcloud compute instances list --project="$PROJECT_ID" --quiet --format="csv(id,name,status,zone,networkInterfaces[].networkIP.list(),networkInterfaces[].accessConfigs[].natIP.list())" \
